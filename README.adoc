...
A placeholder is defined by (respect the) cast: `/*WHERE{ ... }*/`
...
So this SQL query `SELECT * FROM actedIn WHERE born > 1980 AND movie = "The Matrix"` will generate this cypher query :
`MATCH (p:Person) WHERE p.born > 1980 WITH p MATCH (p)-[:ACTED_IN]->(m:Movie) WHERE m.title = "The Matrix" RETURN p.name AS actor, p.born AS born, m.title AS movie`

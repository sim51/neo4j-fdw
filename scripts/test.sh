...
echo "~~~"
docker exec fdw-pg /source/scripts/docker/postgres/init.sh
...
echo
docker exec fdw-neo4j /source/scripts/docker/neo4j/init.sh
...
docker exec fdw-pg /source/scripts/python_test.sh
...
docker exec fdw-pg /source/scripts/pg_regress.sh /source/test
if [ $? -gt 0 ]; then
